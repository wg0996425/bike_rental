...
function sqlAllJoins() {
    return "
        SELECT r.rental_id, b.model, b.type, b.hourly_rate, c.first_name, c.last_name, r.start_time, r.end_time
        FROM rentals r
        JOIN bikes b ON r.bike_id = b.bike_id
        JOIN customers c ON r.customer_id = c.customer_id
...
function sqlUpdateEndTime($rentalId) {
    return "UPDATE rentals SET end_time = NOW() WHERE rental_id = $rentalId;";
}

function sqlUpdateBikeStatus($bikeId) {
    return "UPDATE bikes SET available = 0 WHERE bike_id = $bikeId;";
}
?>
